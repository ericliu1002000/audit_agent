...
from indicator_audit.models import AuditBatch
from indicator_audit.services.strategies import BaseAuditStrategy
from indicator_audit.services.strategies.declaration import DeclarationAuditStrategy
from indicator_audit.services.strategies.self_eval import SelfEvalAuditStrategy
from indicator_audit.services.utils.excel_to_markdown import parse_excel_to_markdown


logger = logging.getLogger(__name__)


def build_strategy(audit_type: str | None) -> BaseAuditStrategy:
    """
    功能说明:
        根据审核类型返回对应的策略实现，未知类型将回退到申报表策略。
    使用示例:
        strategy = build_strategy(AuditBatch.AUDIT_TYPE_DECLARATION)
    输入参数:
        audit_type: 审核类型标识（如 declaration / self_eval）。
    输出参数:
        BaseAuditStrategy: 可执行抽取与校验的策略实例。
    """

    if audit_type == AuditBatch.AUDIT_TYPE_SELF_EVAL:
        return SelfEvalAuditStrategy()
    return DeclarationAuditStrategy()
...
def audit_project_file(
    ...
    strategy = build_strategy(audit_type)
    ...
    try:
        ...
        pydantic_data = strategy.extract_data(markdown_text)

        _maybe_log("processing", "正在进行资金与逻辑校验...")
        rigid_raw_issues = strategy.run_rigid_validation(pydantic_data)
        ...
        if len(critical_after_rigid) < 5:
            ...
            semantic_raw_issues = strategy.run_semantic_check(pydantic_data)
        ...
        # 系统级错误
        logger.error("Audit system error: %s", e, exc_info=True)
        return {
            "success": False,
