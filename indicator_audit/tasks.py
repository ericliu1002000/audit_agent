...
from indicator_audit.services.core import audit_pipeline
...
def run_audit_task(
    ...
    try:
        try:
            audit_file = AuditFile.objects.select_related("batch").get(pk=audit_file_id)
        ...
        if audit_type not in (
            ...
            audit_type = (
                audit_file.batch.audit_type
                if audit_file and audit_file.batch_id
                else AuditBatch.AUDIT_TYPE_DECLARATION
            )
        strategy = audit_pipeline.build_strategy(audit_type)
        ...
        rigid_issues = strategy.run_rigid_validation(pydantic_data)
        ...
        if critical_count < 5:
            ...
            semantic_issues = strategy.run_semantic_check(pydantic_data)
